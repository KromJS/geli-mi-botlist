...
module.exports = {
	...
	run: async(client, message, args) => {
     ...
     .setFooter(client.user.username, client.user.avatarURL())
	  ...
	  let botID = args[0];
      if(!botID || isNaN(botID)) return message.channel.send(embed.setDescription("Lütfen Eklemek İstediğiniz Botun ID sini Giriniz."));
	  let discordBot = null;
      try {
	  ...
	  if(bot) {
        ...
        return message.channel.send(` **${discordBot.username}** Adlı Bot Sisteme **${member.username}** Tarafından Eklenmiş Durum; **${bot.status}**`)
	 }
